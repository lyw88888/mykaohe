...
.prev-btn>button:hover {
    background-color: #45a049 !important; /* 深绿色 */
}
.next-btn>button {
...
.next-btn>button:hover {
    background-color: #0b7dda !important; /* 深蓝色 */
}
/* 标题样式调整 */
h1, h2, h3, h4, h5, h6 {
...
if page == "项目介绍":
    ...
    with col_sidebar:
        ...
        # 构建img文件夹下的图片路径
        img_list = [
            os.path.join(IMG_DIR, "1.png"),  # img/1.png
            os.path.join(IMG_DIR, "2.png")   # img/2.png
        ]  
        ...
        if not os.path.exists(IMG_DIR):
            os.makedirs(IMG_DIR)
            st.warning(f"⚠️ 图片文件夹 {IMG_DIR} 不存在，已自动创建，请将1.png和2.png放入该文件夹")
        ...
        try:
            ...
            if os.path.exists(current_img):
                ...
                # 文件不存在时提示
                st.warning(f"⚠️ 图片文件 {current_img} 未找到")
                # 显示占位符图片
                st.image("https://via.placeholder.com/200x150?text=图片未找到", 
                         caption=f"{current_caption}（文件缺失）", width=200)
        except Exception as e:
            # 捕获所有图片加载错误
            st.warning(f"❌ 图片加载失败: {str(e)[:50]}")
            # 显示占位符图片
            st.image("https://via.placeholder.com/200x150?text=图片加载失败", 
                     caption=current_caption + "（加载失败）", width=200)
...
# 页面3：成绩预测页
    ...
    try:
        ...
        st.error(f"❌ 模型文件 {MODEL_PATH} 未找到，请确认文件路径是否正确")
    except pickle.UnpicklingError:
        st.error(f"❌ 模型文件 {MODEL_PATH} 损坏或格式错误")
    except Exception as e:
        st.error(f"❌ 模型加载失败: {str(e)[:100]}")

    # 预测按钮点击事件（仅当模型加载成功时可点击）
    if model and st.button("预测期末成绩", type="primary", help="点击预测期末成绩"):
        # 构造模型输入数据（二维数组）
        ...
        try:
            ...
            st.error(f"❌ 预测失败: {str(e)[:100]}")
    elif not model:
        # 模型未加载时禁用按钮
        st.button("预测期末成绩", type="primary", disabled=True)
        st.warning("⚠️ 请先确保模型文件正确加载后再进行预测")
